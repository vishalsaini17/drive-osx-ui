...
interface EditorTabBarProps {
  ...
  onNew: () => void;
  /** Rendered before the first tab (the drawer toggle on phone/tablet). */
  leading?: React.ReactNode;
  /** No hover on this device (or a phone/tablet layout): close buttons must always be reachable. */
  alwaysShowClose?: boolean;
}

export default function EditorTabBar({ tabs, activeTabId, onSelect, onClose, onNew, leading, alwaysShowClose = false }: EditorTabBarProps) {
      ...
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            ...
            {tab.isDirty ? (
              <Circle
                className={`w-2 h-2 shrink-0 fill-current text-[var(--wb-fg)]/70 ${alwaysShowClose ? '' : 'group-hover:hidden'}`}
                onClick={(e) => {
                  e.stopPropagation();
                }}
              />
            ...
            <button
              ...
              className={`shrink-0 rounded hover:bg-[var(--wb-fg)]/15 items-center justify-center ${
                alwaysShowClose ? 'w-6 h-6 flex' : `w-4 h-4 ${tab.isDirty ? 'hidden group-hover:flex' : 'flex'}`
              }`}
            >
              <X className="w-3 h-3" />
            </button>
