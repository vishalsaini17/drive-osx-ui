...
const CATEGORIES: { id: ConverterCategory; name: string }[] = [
  ...
  { id: 'storage', name: 'Storage1111' },
];

export default function UnitConverter() {
  const { ref: containerRef, isCompact } = useContainerWidth<HTMLDivElement>(620);
  ...
  const convertedResult = useMemo(() => {
    ...
    if (Math.abs(result) < 0.00001 && result !== 0) return result.toExponential(6);
  ...
  return (
    <div ref={containerRef} className="flex-1 flex flex-col p-4 gap-4 max-w-2xl mx-auto w-full select-none overflow-y-auto">
      <div className={`grid ${isCompact ? 'grid-cols-2' : 'grid-cols-5'} gap-1.5 p-1.5 bg-zinc-100 dark:bg-zinc-800/60 rounded-2xl border border-zinc-200 dark:border-zinc-800 text-xs`}>
        {CATEGORIES.map((item) => (
          <button
            key={item.id}
      ...
      <div className="p-5 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800 shadow-md flex flex-col gap-4">
        ...
        <div className={`grid ${isCompact ? 'grid-cols-1' : 'grid-cols-2'} gap-4 items-center relative`}>
          <div className="flex flex-col gap-1.5 p-3.5 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200/80 dark:border-zinc-700/60">
            ...
            {category === 'temperature' ? (
              <select value={fromUnitId} onChange={(event) => setFromUnitId(event.target.value)} className="mt-1 p-2 rounded-lg bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 text-xs font-semibold cursor-pointer">
              ...
              <select value={fromUnitId} onChange={(event) => setFromUnitId(event.target.value)} className="mt-1 p-2 rounded-lg bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 text-xs font-semibold cursor-pointer">
                {activeUnitsList.map((unit) => <option key={unit.id} value={unit.id}>{unit.name} ({unit.symbol})</option>)}
              </select>
            )}
          </div>

          <div className={isCompact ? 'flex justify-center' : 'flex justify-center absolute left-1/2 -translate-x-1/2 z-10'}>
            <button onClick={handleSwap} className="p-3 rounded-full bg-purple-600 text-white hover:bg-purple-500 shadow-md transition-transform hover:scale-110 cursor-pointer" title="Swap From and To units">
          ...
          <div className="flex flex-col gap-1.5 p-3.5 bg-purple-500/5 dark:bg-purple-950/20 rounded-xl border border-purple-500/30">
            ...
            <div className="w-full font-mono text-2xl font-bold text-purple-600 dark:text-purple-300 truncate">{convertedResult}</div>
            {category === 'temperature' ? (
              <select value={toUnitId} onChange={(event) => setToUnitId(event.target.value)} className="mt-1 p-2 rounded-lg bg-white dark:bg-zinc-900 border border-purple-500/30 text-xs font-semibold cursor-pointer">
                <option value="C">Celsius (°C)</option>
                <option value="F">Fahrenheit (°F)</option>
                <option value="K">Kelvin (K)</option>
              </select>
        ...
        <div className="p-3 rounded-xl bg-zinc-50 dark:bg-zinc-800/40 border border-zinc-200 dark:border-zinc-800 text-xs flex items-center gap-2 text-zinc-500 dark:text-zinc-400">
          ...
          <span>Conversion: {displayedInput} {fromUnitId} = {convertedResult} {toUnitId}</span>
        </div>
      </div>
    </div>
  );
