...
export default function TopBar({ settings, setSettings, currentDesktop, setCurrentDesktop }: TopBarProps) {
  ...
  return (
    <div id="os-topbar" className="absolute top-0 left-0 right-0 h-10 bg-black/15 backdrop-blur-md border-b border-white/5 flex items-center justify-between px-5 text-sm text-white select-none z-[9999]">
      ...
      <div className="relative">
        <button
          ...
          className="flex items-center gap-1.5 hover:bg-white/10 px-3 py-1 rounded-md transition-colors font-medium cursor-pointer"
        >
        ...
        {isDesktopDropdownOpen && (
          <>
            ...
            <div className="absolute left-0 mt-1.5 w-44 bg-[#1e1b4b]/80 backdrop-blur-2xl border border-white/10 rounded-xl p-1.5 shadow-2xl z-20 animate-in fade-in slide-in-from-top-1">
              {[1, 2, 3].map((num) => (
                <button
                  key={num}
                  onClick={() => {
                  ...
                  className="w-full flex items-center justify-between text-left px-3 py-1.5 rounded-lg hover:bg-white/10 transition-colors text-xs"
                >
                  <span>Desktop Workspace {num}</span>
                  {currentDesktop === num && <Check className="w-3.5 h-3.5 text-pink-400" />}
                </button>
              ))}
            </div>
          </>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative">
          <button
            ...
            className="hover:bg-white/10 p-1.5 rounded-md transition-colors cursor-pointer"
            title="Wi-Fi Status"
          >
          ...
          {isWifiDropdownOpen && (
            <>
              ...
              <div className="absolute right-0 mt-1.5 w-60 bg-[#1e1b4b]/80 backdrop-blur-2xl border border-white/10 rounded-xl p-3 shadow-2xl z-20 animate-in fade-in slide-in-from-top-1">
        ...
        <div className="relative">
          ...
          {isVolumeDropdownOpen && (
            <>
              ...
              <div className="absolute right-0 mt-1.5 w-56 bg-[#1e1b4b]/80 backdrop-blur-2xl border border-white/10 rounded-xl p-3.5 shadow-2xl z-20 animate-in fade-in slide-in-from-top-1">
                <div className="flex items-center justify-between mb-2">
                  <span className="font-semibold text-xs">Volume Control</span>
                  <span className="text-xs text-white/60">{settings.soundsEnabled ? `${settings.volume}%` : 'Muted'}</span>
                </div>
                ...
                <div className="mt-2.5 pt-2 border-t border-white/5 flex items-center justify-between">
                  <label className="text-[11px] text-white/60 flex items-center gap-1.5 cursor-pointer">
                    <input
                      type="checkbox"
        ...
        <div className="relative">
          <button
            ...
            className="hover:bg-white/10 p-1.5 rounded-md transition-colors flex items-center gap-1 cursor-pointer"
          ...
          {isBatteryDropdownOpen && (
            <>
              ...
              <div className="absolute right-0 mt-1.5 w-56 bg-[#1e1b4b]/80 backdrop-blur-2xl border border-white/10 rounded-xl p-3 shadow-2xl z-20 animate-in fade-in slide-in-from-top-1 text-xs">
                ...
                <div className="mt-2.5 pt-2 border-t border-white/5 flex justify-between">
                  <button 
                    ...
                    className="w-full text-center py-1 hover:bg-white/5 text-[10px] text-pink-300 rounded"
                  >
