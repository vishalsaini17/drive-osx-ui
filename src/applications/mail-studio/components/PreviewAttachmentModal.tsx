    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs animate-in fade-in duration-150">
      <div className={`w-full max-w-xl rounded-2xl shadow-2xl border flex flex-col overflow-hidden max-h-[85vh] ${
        ...
        <div className={`p-4 border-b flex items-center justify-between ${
          ...
          <button
            onClick={onClose}
            className="p-1 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-white cursor-pointer shrink-0 ml-2"
          >
            <X size={16} />
          </button>
        </div>
        ...
        {/* Content Preview */}
        <div className="p-6 flex-1 overflow-y-auto flex flex-col items-center justify-center text-center gap-4 bg-slate-900/5 dark:bg-black/20 min-h-[240px]">
          {attachment.type.toUpperCase().includes('IMAGE') || attachment.name.endsWith('.png') || attachment.name.endsWith('.jpg') ? (
            <div className="w-full h-48 rounded-xl bg-slate-200 dark:bg-white/10 flex flex-col items-center justify-center gap-2 border border-dashed border-slate-400 dark:border-white/20">
              <span className="text-3xl">🖼️</span>
              <span className="text-xs font-semibold text-slate-500 dark:text-white/60">Image Attachment Preview</span>
            </div>
          ) : attachment.type.toUpperCase().includes('PDF') || attachment.name.endsWith('.pdf') ? (
            <div className="w-full p-4 rounded-xl bg-white dark:bg-[#18181b] border text-left shadow-xs">
              <div className="text-xs font-bold text-blue-600 mb-1 flex items-center gap-1.5">
                <FileText size={14} />
                <span>PDF Specification Document</span>
              </div>
              <p className="text-xs text-slate-600 dark:text-white/70 leading-relaxed font-mono bg-slate-50 dark:bg-black/30 p-3 rounded-lg border dark:border-white/10">
                {attachment.content || `%PDF-1.4 Spec Preview\nDocument Title: ${attachment.name}\nSize: ${attachment.size}`}
              </p>
            </div>
          ) : (
            <div className="w-full p-4 rounded-xl bg-white dark:bg-[#18181b] border text-left font-mono text-xs text-slate-700 dark:text-white/80 whitespace-pre-wrap">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer ${
              savedToDrive
                ...
                : 'bg-slate-200 dark:bg-white/10 hover:bg-slate-300 dark:hover:bg-white/20 text-slate-800 dark:text-white'
            }`}
          >
            {savedToDrive ? (
              <>
